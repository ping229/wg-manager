...
四种独立部署模式:
1. Portal  - 用户门户，可独立部署，需配置 ADMIN_URL
2. Admin   - 管理后台 + 数据库，可独立部署
3. Agent   - VPN 节点代理，可独立部署到多个节点
4. 单机模式 - 所有服务部署在一台服务器（默认）
```
...
Portal 独立部署时，需要在 `.env` 中配置 `ADMIN_URL` 指向 Admin 管理后台地址。
...
Admin 独立部署时，数据库也在该服务器上，Agent 需要配置连接到此 Admin。

### Agent 部署
仅部署 Agent（节点代理）服务，在 VPN 节点上运行。

```bash
sudo bash install.sh --agent
```

Agent 需要在 Admin 管理后台注册后才能使用。
